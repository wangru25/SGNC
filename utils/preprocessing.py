...
LastEditTime: 2022-11-04 11:52:54
Email: wangru25@msu.edu
FilePath: /FokkerPlanckAutoEncoder/utils/preprocessing.py
Description: Functions that can be used to preprocess SMILES sequnces in the form used in the publication.
'''
import numpy as np
import pandas as pd
...
def canonical_removess_smiles(sml):
    try:
        sml_c=canonical_smile(sml)
    except:
        sml_c=sml
        
    sml_cr=remove_salt_stereo(sml_c, REMOVER)
    if(sml_cr==np.float("nan")):
        return(sml_c)
    else:
        return(sml_cr)
...
def preprocess_smiles(sml):
    ...
    """
    c_sml=canonical_smile(sml)
    new_sml = remove_salt_stereo(c_sml, REMOVER)
